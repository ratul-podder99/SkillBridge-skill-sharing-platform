...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        try {
            ...
            // FIXED: Verify using enrollments.tutor_id directly instead of joining through skill_offerings
            $verifyStmt = $db->prepare("
                SELECT e.*, so.skill_title
                FROM enrollments e
                LEFT JOIN skill_offerings so ON e.skill_id = so.skill_id
                WHERE e.enrollment_id = ? AND e.tutor_id = ?
            ");
...
try {
    ...
    // FIXED: Build WHERE clause using enrollments.tutor_id directly
    $whereConditions = ["e.tutor_id = ?"];
    $params = [$userId];
    ...
    // FIXED: Get enrollments using enrollments.tutor_id
    ...
    // FIXED: Get statistics using enrollments.tutor_id
...
<body class="bg-light">
    ...
    <div class="page-header">
        <div class="container">
            <h1 class="mb-2">
                <i class="fas fa-users-cog me-3"></i>Manage Enrollments
            </h1>
